...
function ProductScopePicker({ selected, onChange }) {
  ...
  const add = (product) => {
    onChange([...selected, { id: Number(product.id), key: product.key, sku: product.sku, nameRo: product.name }]);
    ...
    <div className="adm-promo-product-picker">
      <div className="adm-promo-scope-selected">
        {selected.map((product) => (
          <span key={product.id}>{product.nameRo || product.name || product.sku || product.key || product.id}
            <button type="button" onClick={() => onChange(selected.filter((item) => Number(item.id) !== Number(product.id)))} aria-label="Elimină produsul"><X size={13}/></button>
          </span>
        ))}
        {!selected.length && <small>Nu sunt produse selectate</small>}
      </div>
      <div className="adm-promo-product-search">
        <input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); search(); } }} placeholder="Caută după nume, SKU sau brand" aria-label="Caută produse pentru promoție"/>
        <button className="adm-compact-btn" type="button" onClick={search} disabled={query.trim().length < 2 || loading}>{loading ? <LoaderCircle className="adm-spin" size={15}/> : <Search size={15}/>}Caută</button>
      </div>
      {error && <div className="adm-inline-error" role="alert"><AlertTriangle size={14}/>{error}</div>}
      {results.length > 0 && <div className="adm-promo-search-results">
        {results.map((product) => <button type="button" key={product.id} onClick={() => add(product)}><span><b>{product.name}</b><small>{product.sku || product.key} · {product.categoryName}</small></span><Plus size={15}/></button>)}
      </div>}
    </div>
  );
}

function PromoEditor({ promo, creating, categories, loading, error, onClose, onSave, onDeactivate }) {
      ...
      <aside className="adm-drawer adm-promo-drawer" ref={dialogRef} role="dialog" aria-modal="true" aria-label="Editor cod promo" tabIndex={-1}>
        ...
        {promo && !loading && <form onSubmit={submit} className="adm-promo-form">
          ...
          <div className="adm-form-section">
            ...
            <p className="adm-form-hint">Fără categorii și produse selectate, codul se aplică întregului coș. Dacă selectezi ambele, se aplică reuniunii lor.</p>
            <div className="adm-promo-category-grid">{categories.map((category) => <label className="adm-check" key={category.id}><input type="checkbox" checked={categoryIds.includes(category.id)} onChange={() => toggleCategory(category.id)}/><span>{category.nameRo || category.name_ro || category.id}</span></label>)}</div>
          </div>
          <div className="adm-form-section"><h3>Produse eligibile</h3><ProductScopePicker selected={products} onChange={setProducts}/></div>
          ...
          {saveError && <div className="adm-list-message adm-error"><AlertTriangle size={18}/>{saveError}</div>}
          <footer className="adm-promo-actions">
            ...
            <button className="adm-primary" type="submit" disabled={saving}>{saving ? <LoaderCircle className="adm-spin" size={17}/> : <Check size={17}/>}Salvează</button>
          </footer>
        </form>}
      </aside>
    </div>
...
export default function AdminPromos({ onUnauthorized }) {
  ...
  React.useEffect(() => {
    adminRequest('/api/admin/categories?state=active').then((payload) => setCategories(payload.items)).catch((caught) => {
      if (caught instanceof AdminApiError && [401, 403].includes(caught.status)) onUnauthorized?.();
    });
  }, [onUnauthorized]);
  ...
  const create = () => { setCreating(true); setSelectedId('new'); setDetail(emptyPromo()); setDetailError(''); };
  ...
  return (
    <>
      <header className="adm-topbar adm-promos-topbar"><div><span>Vânzări</span><h1>Coduri promo</h1></div><div><button className="adm-icon-btn" type="button" onClick={load} title="Actualizează"><RefreshCw className={loading ? 'adm-spin' : ''} size={19}/></button><button className="adm-primary" type="button" onClick={create}><Plus size={18}/>Cod nou</button></div></header>
      ...
      {selectedId && <PromoEditor key={selectedId} promo={detail} creating={creating} categories={categories} loading={detailLoading} error={detailError} onClose={close} onSave={save} onDeactivate={deactivate}/>}
    </>
  );
}
