 ...
 * Usage: node scripts/migrate-drive-r2.mjs --public-base-url <url>
 *   --environment <preview|production> --confirm-bucket <bucket>
...
const BASE = requirePublicBaseUrl(cliArgs, process.env, target.environment);
...
if (si < 0 || pi < 0) throw new Error('photos.csv must contain SKU and Photo columns');

const urls = new Set();
for (const r of rows.slice(1)) for (const u of (r[pi] || '').split(/\s+/).filter(Boolean)) if (isDrive(u)) urls.add(u);
console.log(`unique Drive URLs for ${target.environment}: ${urls.size}`);
...
for (const u of urls) {
  try {
    ...
    remap[u] = `${BASE}/${key}`;
    console.log(`  ${exists ? 'exists' : 'uploaded'} ${u.slice(0, 60)}… -> ${key} (${buf.length}b)`);
  } catch (e) {
    failures.push({ url: u, error: String(e?.message || e) });
    console.error(`  FAIL ${u} : ${e.message}`);
  }
}

if (failures.length) {
  console.error(`Migration failed for ${failures.length} URL(s); photos.csv was not rewritten.`);
  process.exitCode = 1;
} else {

  // Rewrite only after every download/upload succeeded, so a failed run cannot
  // leave a partially migrated tracked source file.
  let changed = 0;
  const out = [['SKU', 'Photo']];
